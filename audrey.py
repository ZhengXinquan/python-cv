...
charade = VideoFileClip("./input/761.mp4")
tfreeze = cvsecs(5.21) # Time of the freeze, 19'21

# when using several subclips of a same clip, it can be faster
# to create 'coreaders' of the clip (=other entrance points).
clip_before = charade.subclip(tfreeze -3,tfreeze)
clip_after = charade.subclip(tfreeze ,tfreeze +3)
...
final_clip =  concatenate_videoclips([ clip_before,
                            painting_fading.set_duration(2),
                            clip_after])

final_clip.write_videofile('./output/audrey.avi',fps=charade.fps,
